...
const fetchCarbonIntensity = async (
  ...
  try {
    const url = `${
      EU_ADAPTER_CONFIG.baseUrl
    }/carbon-intensity/latest?zone=${countryCode.toUpperCase()}`
    ...
    if (!response.ok) {
      ...
      if (response.status === 401) {
        throw new Error(
          "Invalid Electricity Maps API key. Please check your API key configuration."
        )
...
const fetchPowerBreakdown = async (
  ...
  try {
    const url = `${
      EU_ADAPTER_CONFIG.baseUrl
    }/power-breakdown/latest?zone=${countryCode.toUpperCase()}`
    const response = await fetch(url, {
      headers: { "auth-token": apiKey },
    })
...
const transformPowerBreakdownToMix = (
  ...
  for (const [fuelType, percentage] of Object.entries(percentages)) {
    const mappedFuel = EU_ADAPTER_CONFIG.fuelTypeMapping[fuelType.toLowerCase()] || "other"
